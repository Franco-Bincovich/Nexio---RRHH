...
import { guardarPerfil, subirAvatar, subirDocumento, eliminarDocumento } from "./actions";
import { guardarPreferenciasNotif } from "@/app/dashboard/empleado/notificaciones/actions";
...
export default function PerfilClient({ empleado, userId, documentos: initialDocs, notifPrefs: initialPrefs }: Props) {
  ...
  async function handleGuardarPerfil(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await guardarPerfil(fd);
      setFormMsg(res);
  ...
  async function handleSubirDoc(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      ...
      if (res.error) setDocMsg({ error: res.error });
        ...
        setDocMsg({ ok: true });
        router.refresh();
        // Agregar el doc a la lista local
        setDocs((prev) => [
  ...
  function handleTogglePref(key: keyof NotifPrefs) {
    ...
    setPrefsMsg(null);
    startTransition(async () => {
      const res = await guardarPreferenciasNotif(updated);
      setPrefsMsg(res);
    });
  }
  ...
  async function handleEliminarDoc(doc: Documento) {
    ...
    const res = await eliminarDocumento(path);
    setDeletingDoc(null);
    if (res.error) setDocMsg({ error: res.error });
