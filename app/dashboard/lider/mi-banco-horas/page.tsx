...
export default async function MiBancoHorasLiderPage() {
  ...
  const horasLaborables = (lider.horas_laborables ?? 8) * 60;
  const ajuste          = lider.banco_horas_ajuste ?? 0;
      ...
      <div className="bg-surface rounded-xl border border-[#1A2235] shadow-[0_1px_4px_rgba(0,0,0,0.4)] overflow-hidden">
        ...
        {filas.length === 0 ? (
          <div className="px-5 py-10 text-center">
            <p className="text-sm text-secondary/60">No hay registros con entrada y salida registradas.</p>
            <p className="text-xs text-secondary/40 mt-1">El banco se calcula solo cuando hay hora de entrada y salida.</p>
          </div>
        ) : (
                ...
                {filas.map((f) => {
                  ...
                  return (
                    <tr key={f.id} className="border-b border-[#1A2235] last:border-0 hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-5 py-3 text-right font-medium">{minutosAHorasStr(f.trabajados).replace("+", "")}</td>
                      <td className="px-5 py-3 text-right text-secondary">{minutosAHorasStr(f.laborables).replace("+", "")}</td>
