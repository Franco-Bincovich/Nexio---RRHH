...
const SUGERENCIAS = [
  "¿Quién faltó más este mes?",
  "¿Cómo va el progreso de objetivos?",
  "¿Cuántas horas extra acumuló el equipo?",
...
export default function AiChatClient({ areaId, empresaId, areaNombre }: Props) {
  ...
  async function sendMessage(text: string) {
    ...
    try {
      const res = await fetch("/api/ai/lider", {
        method:  "POST",
        headers: { "content-type": "application/json" },
        body:    JSON.stringify({ messages: newMessages, areaId, empresaId }),
      });
    ...
    <div className="flex flex-col h-[calc(100vh-0px)] lg:h-screen max-h-screen">
      ...
      <div className="flex-shrink-0 border-b border-[#1A2235] px-6 py-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-secondary">{areaNombre}</p>
        </div>
      </div>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="flex flex-col items-center text-center mb-10 pt-4">
              ...
              <p className="text-secondary text-sm mt-1 max-w-md">
                Asistente inteligente con datos reales de tu área. Preguntame lo que necesitás.
              </p>
            </div>
