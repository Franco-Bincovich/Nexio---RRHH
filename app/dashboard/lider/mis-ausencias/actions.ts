...
export async function solicitarAusenciaLider(data: {
  fecha:   string;
  subtipo: string;
  motivo:  string;
}) {
  try {
    ...
    await notificarGerentes(
      lider.empresa_id,
      `El líder ${lider.nombre} solicitó una inasistencia para el ${data.fecha}.`
    );

    revalidatePath("/dashboard/lider/mis-ausencias");
    return { ok: true };
  } catch (e: unknown) {
