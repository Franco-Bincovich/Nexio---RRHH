...
import { solicitarRetiroGerente } from "./actions";
...
export default function MisRetirosGerenteClient({ retiros }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await solicitarRetiroGerente({ fecha, hora_retiro: hora, motivo: motivo.trim() });
        ...
        <form onSubmit={handleSubmit} className="bg-surface border border-[#1A2235] rounded-xl p-5 mb-6 shadow-[0_1px_4px_rgba(0,0,0,0.4)]">
          ...
          {success && (
            <div className="flex items-center gap-2 text-accent text-xs bg-accent/10 border border-accent/20 rounded-lg px-3 py-2 mt-3">
              <CheckCircle2 size={13} />Retiro registrado automáticamente.
            </div>
          )}
      ...
      {retiros.length === 0 ? (
        ...
        <div className="space-y-3">
          {retiros.map((r) => {
            ...
            return (
              <div key={r.id} className="bg-surface rounded-xl border border-[#1A2235] shadow-[0_1px_4px_rgba(0,0,0,0.4)] p-4">
                ...
                {r.aprobador_nombre && (
                  <p className="text-[11px] text-secondary/50">
                    Aprobado por {r.aprobador_nombre}
                  </p>
                )}
              </div>
            );
