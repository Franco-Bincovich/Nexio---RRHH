...
import { Camera, Save, Loader2, CheckCircle2, AlertCircle, Upload, FileText, Trash2, Bell } from "lucide-react";
import { guardarPerfilGerente, subirAvatarGerente, subirDocumentoGerente, eliminarDocumentoGerente, guardarPreferenciasNotifGerente } from "./actions";

interface Empleado { id: string; nombre: string; email: string; telefono: string | null; direccion: string | null; contacto_emergencia_nombre: string | null; contacto_emergencia_telefono: string | null; avatar_url: string | null; }
interface Documento { name: string; created_at: string; id: string; updated_at: string; }
interface NotifPrefs { inasistencias: boolean; objetivos: boolean; vacaciones: boolean; capacitaciones: boolean; }
interface Props { empleado: Empleado; userId: string; documentos: Documento[]; notifPrefs: NotifPrefs; }

export default function PerfilGerenteClient({ empleado, userId, documentos: initialDocs, notifPrefs: initialPrefs }: Props) {
  ...
  const initials = empleado.nombre.split(" ").slice(0, 2).map((n) => n[0]).join("").toUpperCase();

  async function handleGuardarPerfil(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault(); setFormMsg(null);
    const fd = new FormData(e.currentTarget);
    startTransition(async () => { const res = await guardarPerfilGerente(fd); setFormMsg(res); if (res.ok) router.refresh(); });
  }
  async function handleAvatarChange(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0]; if (!file) return;
    setAvatarMsg(null); setAvatarPreview(URL.createObjectURL(file));
    const fd = new FormData(); fd.append("avatar", file);
    startTransition(async () => {
      const res = await subirAvatarGerente(fd);
  ...
  async function handleSubirDoc(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0]; if (!file) return;
    setDocMsg(null); const fd = new FormData(); fd.append("documento", file);
    startTransition(async () => {
      const res = await subirDocumentoGerente(fd);
      if (res.error) setDocMsg({ error: res.error });
      else { setDocMsg({ ok: true }); router.refresh(); setDocs((prev) => [{ name: file.name, created_at: new Date().toISOString(), id: res.path!, updated_at: new Date().toISOString() }, ...prev]); }
      e.target.value = "";
    });
  }
  function handleTogglePref(key: keyof NotifPrefs) {
    const updated = { ...prefs, [key]: !prefs[key] }; setPrefs(updated); setPrefsMsg(null);
    startTransition(async () => { const res = await guardarPreferenciasNotifGerente(updated); setPrefsMsg(res); });
  }
  async function handleEliminarDoc(doc: Documento) {
    setDeletingDoc(doc.name); const path = `${userId}/${doc.name}`;
    const res = await eliminarDocumentoGerente(path); setDeletingDoc(null);
    if (res.error) setDocMsg({ error: res.error });
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="bg-surface rounded-xl border border-border shadow-sm p-6">
        ...
        <div className="flex items-center gap-5">
          <div className="relative flex-shrink-0">
            <div className="w-20 h-20 rounded-full border-2 border-accent/25 bg-accent/10 overflow-hidden flex items-center justify-center text-2xl font-bold text-accent cursor-pointer" onClick={() => avatarInputRef.current?.click()}>
              {avatarPreview ? <img src={avatarPreview} alt="Avatar" className="w-full h-full object-cover" /> : initials}
            </div>
            <button type="button" onClick={() => avatarInputRef.current?.click()} className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-accent text-base flex items-center justify-center hover:bg-accent/80 transition-colors"><Camera size={13} /></button>
          </div>
          <div>
            <p className="text-sm text-secondary mb-2">JPG o PNG, máximo 3 MB</p>
            <button type="button" onClick={() => avatarInputRef.current?.click()} className="text-xs text-accent hover:underline">Cambiar foto</button>
            {avatarMsg?.ok    && <p className="flex items-center gap-1 text-xs text-accent mt-1"><CheckCircle2 size={12} />Foto actualizada</p>}
            {avatarMsg?.error && <p className="flex items-center gap-1 text-xs text-red-400 mt-1"><AlertCircle size={12} />{avatarMsg.error}</p>}
          </div>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div className="pt-2 border-t border-border">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <Field label="Teléfono" name="contacto_emergencia_telefono" type="tel" defaultValue={empleado.contacto_emergencia_telefono ?? ""} />
              </div>
            </div>
          </div>
      ...
      <div className="bg-surface rounded-xl border border-border shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-2">
          {docMsg?.ok    && <div className="flex items-center gap-1.5 text-xs text-accent py-2"><CheckCircle2 size={12} />Archivo subido</div>}
          {docMsg?.error && <div className="flex items-center gap-1.5 text-xs text-red-400 py-2"><AlertCircle size={12} />{docMsg.error}</div>}
              ...
              <li key={doc.name} className="flex items-center justify-between gap-3 px-6 py-3 hover:bg-border/20">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-secondary/50">{new Date(doc.created_at).toLocaleDateString("es-AR", { day: "numeric", month: "short", year: "numeric" })}</p>
                  </div>
                </div>
                <button type="button" onClick={() => handleEliminarDoc(doc)} disabled={deletingDoc === doc.name} className="text-secondary hover:text-red-400 transition-colors flex-shrink-0 disabled:opacity-50">
        ...
        <div className="px-6 py-4 space-y-3">
          {([{ key: "inasistencias", label: "Inasistencias" }, { key: "objetivos", label: "Objetivos" }, { key: "vacaciones", label: "Vacaciones" }, { key: "capacitaciones", label: "Capacitaciones" }] as { key: keyof NotifPrefs; label: string }[]).map(({ key, label }) => (
            <div key={key} className="flex items-center justify-between gap-4">
              ...
              <button type="button" onClick={() => handleTogglePref(key)} disabled={isPending} className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 disabled:opacity-60 ${prefs[key] ? "bg-accent" : "bg-white/15"}`} aria-checked={prefs[key]} role="switch">
          ...
          {prefsMsg?.ok    && <p className="flex items-center gap-1 text-xs text-accent"><CheckCircle2 size={12} />Preferencias guardadas</p>}
          {prefsMsg?.error && <p className="flex items-center gap-1 text-xs text-red-400"><AlertCircle size={12} />{prefsMsg.error}</p>}
        </div>
      </div>
    </div>
  );
...
function Field({ label, name, type = "text", defaultValue, placeholder, required, disabled }: { label: string; name: string; type?: string; defaultValue?: string; placeholder?: string; required?: boolean; disabled?: boolean; }) {
  return (
    <div>
