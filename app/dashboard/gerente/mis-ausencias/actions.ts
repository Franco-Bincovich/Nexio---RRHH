...
export async function solicitarAusenciaGerente(data: {
  fecha:   string;
  subtipo: string;
  motivo:  string;
}) {
  try {
    ...
    const { error } = await admin.from("solicitudes_ausencia").insert({
    ...
    await notificarOwner(
      gerente.empresa_id,
      `El gerente ${gerente.nombre} registró una inasistencia para el ${data.fecha}.`
    );

    revalidatePath("/dashboard/gerente/mis-ausencias");
    return { ok: true };
  } catch (e: unknown) {
