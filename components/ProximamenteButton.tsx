...
export default function ProximamenteButton({ label, className = "" }: Props) {
  return (
    <button
      disabled
      className={`inline-flex items-center gap-2 opacity-60 cursor-not-allowed rounded-lg px-4 py-2 text-sm font-medium border border-white/10 text-secondary ${className}`}
    >
      {label}
